...
from schema import dict_to_namespace


def get_model_from_run(run_path, step=-1, only_conf=False):
    config_path = os.path.join(run_path, "config.yaml")
    with open(config_path) as fp:  # we don't use inheritance here to avoid recursion
        conf = dict_to_namespace(yaml.safe_load(fp))
    if only_conf:
        return None, conf
